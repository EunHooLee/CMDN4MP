...
from src.rrt_net.rrt_net import RRTNet
...
def main(args):
    ...
    rrt = RRTNet(X, Q, x_init, x_goal, max_samples, r, prc)
    path = rrt.rrt_search()
    # print(path)
    if len(path) == 0: 
        total_cost.append(1000000)
        
    else: 
        total_cost.append(compute_cost(path))
...
if __name__ == '__main__':
    ...
    # with open(args.path + 'eval_dict.pkl','rb') as f:
    #     eval_dict0 = pickle.load(f)
    ...
    # for i in range(20):
    #     # cost_eval_dict[f'{i}'] = {}
    #     # eval_dict[f'{i}']['obstacle'] = []
    #     # eval_dict[f'{i}']['x_init'] = []
    #     # eval_dict[f'{i}']['x_goal'] = []
    #     # cost_eval_dict[f'{i}']['cost'] = []

    #     obstacles = eval_dict0[f'{i}']['obstacle'][0]
    #     x_init = eval_dict0[f'{i}']['x_init'][0]
    #     x_goal = eval_dict0[f'{i}']['x_goal'][0]

    #     # eval_dict[f'{i}']['obstacle'].append(obstacles)
    #     # eval_dict[f'{i}']['x_init'].append(x_init)
    #     # eval_dict[f'{i}']['x_goal'].append(x_goal)
    #     main(obstacles, x_init, x_goal, args)
    # cost_eval_dict['cost'] = total_cost

    # with open(f'cost_eval_dict_{args.key}.pkl','wb') as f:
    #     pickle.dump(cost_eval_dict, f)
    # print(total_cost)