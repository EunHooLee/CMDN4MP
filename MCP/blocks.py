...
class MixtureDensityNetwork(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, n_components, hidden_dim, noise_type=NoiseType.DIAGONAL, fixed_noise_level=None):
        ...
        self.dim_in_without_env = 4
        self.pi_network = nn.Sequential(
            nn.Linear(self.dim_in, hidden_dim),
            nn.ReLU(),
        ...
        self.normal_network = nn.Sequential(
            nn.Linear(self.dim_in_without_env, 32),
            nn.ReLU(),
            nn.Linear(32, 32),
            nn.ReLU(),
            nn.Linear(32, dim_out * n_components + num_sigma_channels)
        )
    ...
    def forward(self, x, eps=1e-6):
        ...
        #
        sub_x = x[:,:self.dim_in_without_env] 
        log_pi = torch.log_softmax(self.pi_network(x), dim=-1) # 실제 논문에서도 log_softmax 사용
        
        normal_params = self.normal_network(sub_x)
    ...
    def loss(self, log_pi, mu, sigma , y):
        ...
        return torch.mean(-loglik) # batch mean loss
