...
class RRTBaseNet(object):
    ...
    def new_and_near(self, tree, q):
        ...
        #     #     x_rand = self.X.obstacle_free_sample()
        #     #     x_nearest = self.get_nearest(tree, x_rand)
        #     #     x_new = self.bound_point(steer(x_nearest, x_rand, q[0]))
        ...
        #     return None, None
        # print(self.X.obstacle_free(x_new))
        self.samples_taken += 1
        return x_new, x_nearest
    ...
    def connect_to_point(self, tree, x_a, x_b):
        ...
        """
        # print(self.X.obstacle_free(x_b))
        # self.r : 충돌 확인용 resolution
        if self.trees[tree].V.count(x_b) == 0 and self.X.obstacle_free(x_b) and self.X.collision_free(x_a, x_b, self.r):
            
            self.add_vertex(tree, x_b)
            self.add_edge(tree, x_b, x_a)
            # 충돌을 벗어나면 0
            self.X.num_collision = 0
            return True
        # 충돌시 : 
        self.X.num_collision += 1
        return False
